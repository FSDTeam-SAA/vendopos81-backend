...
const createOrder = async (payload: any, email: string) => {
  ...
  try {
    ...
    if (!user.isVerified) {
      throw new AppError('Please verify your account to place an order', StatusCodes.FORBIDDEN);
    }
...
const getMyOrders = async (
  ...
  // ✅ Step 1: Get all orderIds
  const orderIds = orders.map((o) => o._id);

  // ✅ Step 2: Fetch all payments in one query
  ...
  // ✅ Step 4: Format orders with dynamic paymentStatus
...
const getAllOrdersForAdmin = async (query: any) => {
  ...
  // ✅ Create payment map
  const paymentMap = new Map(payments.map((p: any) => [p.orderId.toString(), p]));
  ...
  const formattedOrders = orders.map((order: any) => ({
    ...
    paymentStatus:
      paymentMap.get(order._id.toString())?.status === 'success' ? 'paid' : order.paymentStatus,
...
const getOrderFormSupplier = async (email: string, query: any) => {
  ...
  if (!supplier) {
    throw new AppError('You are not registered as a supplier', StatusCodes.FORBIDDEN);
  }
  ...
  const formattedOrders = orders.map((order: any) => ({
    ...
    items: order.items
      .filter(
        (item: any) =>
          item.supplierId && item.supplierId._id.toString() === supplier._id.toString(),
      )
      .map((item: any) => {
        // ✅ wholesale object
        const wholesaleObj = item.wholesaleId as any;
...
const cancelMyOrder = async (orderId: string, email: string) => {
  ...
  try {
    ...
    // 2️⃣ Fetch order
    const order: any = await Order.findById(orderId).populate('items.productId').session(session);
    ...
    if (order.paymentType !== 'cod') {
      throw new AppError('Only COD orders can be cancelled', StatusCodes.BAD_REQUEST);
    }
    ...
    if (order.userId.toString() !== user._id.toString()) {
      throw new AppError('You can only cancel your own orders', StatusCodes.FORBIDDEN);
    }
...
const updateOrderStatus = async (orderId: string, payload: any, email: string) => {
  const session = await mongoose.startSession();
  session.startTransaction();
  ...
  try {
    ...
    const supplier = await JoinAsSupplier.findOne({ userId: user._id }).session(session);
    ...
    const item = order.items.find(
      (i: any) =>
        i._id.toString() === itemId && i.supplierId.toString() === supplier._id.toString(),
    );
    if (!item) throw new AppError('This item does not belong to your supplier', 403);
    ...
    if (statuses.every((s: any) => s === 'ready_to_ship')) order.orderStatus = 'ready_to_ship';
    else if (statuses.every((s: any) => s === 'shipped')) order.orderStatus = 'shipped';
    else if (statuses.every((s: any) => s === 'delivered')) order.orderStatus = 'delivered';
    else if (statuses.every((s: any) => s === 'cancelled')) order.orderStatus = 'cancelled';
    else order.orderStatus = 'partially_shipped';

    // 7️⃣ Update supplier stats for ALL delivered items
    const deliveredItems = order.items.filter((i: any) => i.status === 'delivered');

    // Group by supplier
    const supplierMap: Record<string, { totalOrders: number; totalSales: number }> = {};

    deliveredItems.forEach((i: any) => {
      const supId = i.supplierId.toString();
      if (!supplierMap[supId]) supplierMap[supId] = { totalOrders: 0, totalSales: 0 };
